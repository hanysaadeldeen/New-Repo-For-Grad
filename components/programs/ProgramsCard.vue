<template>
  <div class="flex w-full flex-col justify-between rounded-lg bg-primary p-6 shadow-lg">
    <div class="relative flex items-center justify-between border-b border-gray-200 pb-3">
      <div class="flex items-center space-x-3">
        <img :src="`https://zeropoint.runasp.net${program.image}`" :alt="program.title" class="h-10 w-10 rounded-full"
          width="40" height="40" />
      ...
      <div class="flex items-center gap-4" v-if="usereRole === 'Admin'">
        <nuxt-link :to="localePath(`/programs/${program.id}/edtiProgram`)">
          <span><i
              class="fa-regular fa-pen-to-square cursor-pointer text-2xl font-light text-hookYellow transition-all duration-200 ease-in-out hover:text-hookYellow/70"></i></span>
        </nuxt-link>
        <button @click="emit('delete', program.id)">
          <i
            class="fa-regular fa-trash-can cursor-pointer text-2xl font-light text-red-500 transition-all duration-200 ease-in-out hover:text-red-600"></i>
        </button>
      </div>
    </div>
    ...
    <div class="mt-6">
      <div class="flex items-center space-x-2" v-if="program.goldStandard">
        <span class="rounded-full bg-pink-100 px-2.5 py-1 text-xs font-medium text-pink-600">Gold Standard</span>
      </div>
      <div class="mt-2 flex items-center">
        <span class="mr-2 text-green-500">💰</span>
        <span class="text-lg font-semibold text-white">${{ program.high }}k - ${{ program.low }}k</span>
      </div>
    </div>
    ...
    <div class="mt-4 flex space-x-4 text-sm text-white">
      ...
      <div class="flex items-center text-lg font-bold">
        <svg class="mr-1.5 h-5 w-5 text-paragraph" fill="none" stroke="currentColor" viewBox="0 0 24 24"
      ...
      <div class="flex items-center text-lg font-bold">
        <span class="mb-0.5 mr-1.5 h-2.5 w-2.5 rounded-full bg-green-300"></span>
